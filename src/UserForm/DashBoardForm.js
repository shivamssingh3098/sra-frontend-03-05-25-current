...
const DashboardForm = forwardRef(
  ({ nextStage, prevStage, setCanProceed, onFormSubmit }, ref) => {
    ...
    useEffect(() => {
      console.log("Dashboard form 1:", serviceId);
      if (serviceId) {
        console.log("Dashboard form:", serviceId);
        // Find the service in services.json
        const service = servicesData[0].services.find(
          (s) => s.id === serviceId
        );
        console.log("Dashboard form: service details 0", service);
        if (service) {
          console.log("Dashboard form: service details", service);
