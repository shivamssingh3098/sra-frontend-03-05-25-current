...
import UploadForm12 from "./UploadForm12";
import DashBoardForm12 from "./DashBoardForm12";
...
const ApplicationForm12 = () => {
  ...
  const handleFormSubmit = (responseData) => {
    ...
    if (responseData && responseData._id) {
      ...
      const formattedTime = formatTime(responseData.applyDate || formData.date);
      ...
      setFormData((prev) => ({
        ...
        date: responseData.applyDate || prev.date,
        address: responseData.address || prev.address,
        mobile: responseData.phone || prev.mobile,
        taluka: responseData.taluka || prev.taluka,
        bhukhandNo: bhukhandNo || prev.bhukhandNo,
        village: responseData.village || prev.village,
        district: responseData.city || prev.district,
        wardNo: wardNo || prev.wardNo,
        nagarpalika: nagarpalika || prev.nagarpalika,
        schemeDeveloper: responseData.schemeDeveloper || prev.schemeDeveloper,
        governmentServiceBranch:
          responseData.governmentServiceBranch || prev.governmentServiceBranch,
      }));
    }
  };
  ...
  return (
    <div className="min-h-screen p-4 bg-gray-100 flex justify-center">
      <div className="w-full max-w-4xl bg-white p-6 rounded shadow-md">
        ...
        {currentStage === 1 && (
          <UploadForm12 formData={formData} formId={formData._id} />
        )}
        {currentStage === 2 && (
