...
const ApplicationApproval = ({ formData }) => {
  ...
  useEffect(() => {
    if (applicationInfo) fetchApplicationData(applicationInfo?._id);
  }, [applicationInfo]);
  useEffect(() => {
    if (applicationData) {
      console.log(
        "Department testing : Application data changed :",
        applicationData
      );
    }
  }, [applicationData]);
  ...
  const fetchApplicationData = async (formId) => {
    console.log(
      "Department testing : call api for application information by form id"
    );
    ...
    console.log(
      "Department testing: get api for application information by form id",
      response.data
    );
    ...
    // });
    const signatureObj = data?.documents?.documentList.find((doc) =>
      doc.hasOwnProperty("Signature")
    );
  ...
  useEffect(() => {
    console.log(
      "department testing : checkbox functionality",
      checkedCheckboxes
    );
    const allChecked =
    ...
    if (allChecked) setAcceptRejectBtnEnable(true);
    else setAcceptRejectBtnEnable(false);
  }, [checkedCheckboxes]);
  ...
  const handleAcceptRejectAction = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        CONFIG.API_BASE_URL + "/api/v1/department-managers/create-remark",
        formData,
        config
      );
              ...
              return (
                ...
                >
                  <input
                    ...
                    checked={
                      isAccepted || checkedCheckboxes[labelText] || false
                    }
                    onChange={() => {}}
              ...
              <form onSubmit={handleAcceptRejectAction}>
                ...
                <div className="modal-header">
                  <h5 className="modal-title fw-bold">
                    {action === "ACCEPTED" ? "Accept" : "Reject"} Action
                  </h5>
                  <button
                    type="button"
                    className="btn-close"
